...
def parse(filename, on_parse_header=None):
    ...
    try:
        ...
        if header.version == 1 :
            print("reading file: ",header.name)
            r = r and _read_prototypes(parser, parser.prototypes)
        else:
            errprint("Unknown version: {0}",header.version)
            r = False
    except IOError as e:
...
def _read_prototypes(state, prototypes):
    while not state.stream.eof():
        ...
        size = state.stream.read_uleb128()

        if size == 0:
             break

        if not ljd.rawdump.prototype.read(state, prototype, size):
            if state.stream.eof():
                break
            else:
